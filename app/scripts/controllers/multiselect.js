  ...
  .controller('MultiselectCtrl', function ($scope) {
    $scope.awesomeThings = [
      ...
      'Karma'
        ...
        $scope.pinSelections = function(){
          $.each($scope.mySelections, function(key, val){
            if( $.inArray(val, $scope.pinnedSelections) === -1){
              $scope.pinnedSelections.push(val);
            }
          });
        };
        $scope.remove = function(itm){
          var pos = $scope.pinnedSelections.indexOf(itm);
          $scope.pinnedSelections.splice(pos, 1);
        };
  });
